...
min_max_data %>% 
  select(season, value, min_or_max, league) %>%
  ...
  # make the ggplot, setting fill to min_or_max and position to dodge so that 
  # I get two side-by-side and color-coded columns
  ...
  # can't see any columns other than the MLS maximum salary. I'm still not 100%
  # sure that this is the best rescaling because it makes the extent of the
  # salary differences much less striking. However, lots of googling made it
  # clear that axis breaks are a no-no in ggplot, and I couldn't leave the 
  # graph as it was, so a logorithmic scale seemed to make the most sense. 
  # To mitigate some of the non-intuitiveness of the log scale, I changed the
  # y axis so that values are reported in numbers, not as log funcitons. 
